...
interface SendDialogProps {
  ...
  onSend: (recipients: any[], notifyByEmail: boolean) => void;
  value: any[];
  setValue: (value: any[]) => void;
  selectedValue: string;
  setSelectedValue: (value: string) => void;
  notifyMail: boolean;
  setNotifyMail: (value: boolean) => void;
  checkSender: boolean;
  setCheckSender: (value: boolean) => void;
...
const SendDialog: React.FC<SendDialogProps> = ({
  ...
  const isRecipientSelected = (recipient: any) => {
    return value.some((item) => item.id === recipient.id);
  };

  const handleSave = () => {
    if (selectedValue !== 'School' && value.length === 0) {
      toast({
        title: 'Error',
